  ...
  try {
    ...
    const case1 = await payload.create({
      ...
      data: {
        ...
        solution: 'Разработали интеллектуального чат-бота на базе RAG-архитектуры, который консультирует клиентов по техническим характеристикам продукции, помогает с подбором аналогов и отвечает на типовые вопросы.',
        results: 'Целевой рост автоматизации ответов с 30% до 70%+. Сокращение времени ответа на запросы клиентов с 4 часов до 2 минут. Освобождено 40% времени специалистов поддержки для решения сложных задач.',
    ...
    const case2 = await payload.create({
      ...
      data: {
        ...
        solution: 'Создали Telegram-бота с интеграцией OCR и LLM для распознавания заявок в любом формате. Бот автоматически извлекает данные, формирует структурированную заявку и генерирует коммерческое предложение.',
        results: 'Время обработки заявки сократилось с 2 часов до 10 минут. Устранены ошибки при ручном вводе данных. Увеличена пропускная способность отдела продаж в 3 раза.',
    ...
    const case3 = await payload.create({
      ...
      data: {
        ...
        solution: 'Развернули on-premise AI-платформу на базе открытых LLM моделей в закрытом контуре компании. Система анализирует договоры, выявляет риски и формирует резюме по документам.',
        results: '100% конфиденциальность данных клиентов. Ускорение анализа документов в 5 раз. Юристы получили инструмент для быстрой и безопасной работы с большими объемами документов.',
